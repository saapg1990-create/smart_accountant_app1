...
interface ControlButtonsProps {
  showAdd?: boolean; showEdit?: boolean; showDelete?: boolean;
  showSave?: boolean; showCancel?: boolean; showSearch?: boolean;
  showPrint?: boolean; showRefresh?: boolean; showExport?: boolean;
  showThermalPrint?: boolean;
  onAdd?: () => void; onEdit?: () => void; onDelete?: () => void;
  onSave?: () => void; onCancel?: () => void; onSearch?: () => void;
  onPrint?: () => void; onRefresh?: () => void; onExport?: () => void;
  onThermalPrint?: () => void;
  printLabel?: string;
}

export const ControlButtons: React.FC<ControlButtonsProps> = (props) => {
  const { showAdd, showEdit, showDelete, showSave, showCancel, showSearch, showPrint, showRefresh, showExport, showThermalPrint,
    onAdd, onEdit, onDelete, onSave, onCancel, onSearch, onPrint, onRefresh, onExport, onThermalPrint, printLabel } = props;

  const handlePrint = async () => {
    if (onPrint) { onPrint(); return; }
    Alert.alert('🖨️ طباعة', 'سيتم تصدير البيانات بصيغة PDF للمشاركة');
  };

  const handleExport = async () => {
    if (onExport) { onExport(); return; }
    try {
      await Share.share({ message: 'تم تصدير البيانات من دفتر المحاسب الذكي 📊' });
    } catch(e) {}
  };

  const handleThermalPrint = () => {
    if (onThermalPrint) { onThermalPrint(); return; }
    Alert.alert('🧾 طباعة حرارية', 'جاري تجهيز الطباعة الحرارية...\nتأكد من توصيل الطابعة الحرارية');
  };
  ...
  const toolButtons = [
    ...
    { show: showPrint, icon: '🖨️', label: printLabel || 'طباعة', color: '#F59E0B', bg: '#F59E0B20', border: '#F59E0B40', onPress: handlePrint },
    { show: showThermalPrint, icon: '🧾', label: 'حراري', color: '#EC4899', bg: '#EC489920', border: '#EC489940', onPress: handleThermalPrint },
    { show: showRefresh, icon: '🔄', label: 'تحديث', color: '#06B6D4', bg: '#06B6D420', border: '#06B6D440', onPress: handleRefresh },
    { show: showExport, icon: '📤', label: 'مشاركة', color: '#14B8A6', bg: '#14B8A620', border: '#14B8A640', onPress: handleExport },
  ].filter(b => b.show);

  if (buttons.length === 0 && toolButtons.length === 0) return null;
        ...
        <View style={st.row}>
          {buttons.map((b, i) => (
            <TouchableOpacity key={i} style={[st.btn, { backgroundColor: b.bg, borderColor: b.border }]} onPress={b.onPress}>
              <Text style={st.icon}>{b.icon}</Text><Text style={[st.label, { color: b.color }]}>{b.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
...
const st = StyleSheet.create({
  ...
  addBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#D4AF3720', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#D4AF37' },
