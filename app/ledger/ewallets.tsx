...
export default function EWalletsScreen() {
  ...
  const handleSave = async () => {
    ...
    await addAccount({ name: formData.name, code, type: 'أصل', parentId: '113', balance: parseFloat(formData.balance) || 0, currency: formData.currency, isDebit: 1 });
    await loadAccounts();
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="المحافظ" count={cashBoxes.length} onBack={() => router.back()} onAdd={() => { setFormData({ name: '', currency: 'YER', balance: '0' }); setShowModal(true); }} />
