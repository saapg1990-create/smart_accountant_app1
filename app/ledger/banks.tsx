...
export default function BanksScreen() {
  ...
  const cashBoxes = accounts.filter((a: any) => a.parentId === '112');
  ...
  const handleSave = async () => {
    ...
    if (balance > 0) {
      ...
      Alert.alert('تأكيد', `الرصيد: ${balance.toLocaleString()} ${formData.currency}\nالطبيعة: ${nature}`, [
        ...
        { text: 'حفظ', onPress: async () => {
          ...
          await addAccount({ name: formData.name, code, type: 'أصل', parentId: '112', balance: formData.isDebit ? balance : -balance, currency: formData.currency, isDebit: formData.isDebit ? 1 : 0 });
          await loadAccounts(); resetForm(); setShowModal(false);
          Alert.alert('✅', `تم إضافة ${formData.name} تحت البنك`);
        }}
      ]);
      return;
    }
    ...
    const code = generateCode('112');
    await addAccount({ name: formData.name, code, type: 'أصل', parentId: '112', balance: 0, currency: formData.currency, isDebit: 1 });
    await loadAccounts(); resetForm(); setShowModal(false);
  };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="البنوك" count={cashBoxes.length} onBack={() => router.back()} onAdd={() => { resetForm(); setShowModal(true); }} />
