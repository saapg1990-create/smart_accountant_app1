...
export default function BanksScreen() {
  ...
  const [formData, setFormData] = useState({
    name: '', accountNumber: '', currency: 'YER', balance: '0', isDebit: true, showDebitCredit: false
  });

  useFocusEffect(useCallback(() => { loadAccounts(); }, []));

  const banks = accounts.filter((a: any) => a.parentId === '112');
  const total = banks.reduce((s: number, b: any) => s + (b.balance || 0), 0);
  const count = banks.length + 1;

  const resetForm = () => setFormData({ name: '', accountNumber: '', currency: 'YER', balance: '0', isDebit: true, showDebitCredit: false });
    ...
    if (balance > 0) {
      ...
      Alert.alert('تأكيد', `الرصيد: ${balance.toLocaleString()} ${formData.currency}\nالطبيعة: ${nature}`, [
        ...
        { text: 'حفظ', onPress: async () => {
          await addAccount({ name: formData.name, code, type: 'أصل', parentId: '112', balance: finalBalance, currency: formData.currency, isDebit: formData.isDebit ? 1 : 0, bankAccount: formData.accountNumber });
          await loadAccounts(); resetForm(); setShowModal(false);
          Alert.alert('✅', `تم إضافة ${formData.name} تحت البنوك`);
        }}
      ]);
      return;
    }

    await addAccount({ name: formData.name, code, type: 'أصل', parentId: '112', balance: 0, currency: formData.currency, isDebit: 1, bankAccount: formData.accountNumber });
    await loadAccounts(); resetForm(); setShowModal(false);
  };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="البنوك" count={banks.length} onBack={() => router.back()} onAdd={() => { resetForm(); setShowModal(true); }} />
      ...
      {banks.length === 0 ? <Text style={st.et}>لا توجد بنوك</Text> :
        <FlatList data={banks.filter(b => b.name?.includes(searchQuery))} keyExtractor={i => i.id} renderItem={({ item }) => (
          <View style={st.rc}><Text style={st.rn}>🏦 {item.name}</Text><Text style={st.rd}>🏦 {item.bankAccount || '-'} | الرصيد: {item.balance?.toLocaleString()} {item.currency}</Text></View>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        ...
        <ScrollView style={st.mb}>
          <Text style={st.fl}>الرقم</Text><TextInput style={[st.fi,{color:'#D4AF37'}]} value={`BNK-${count.toString().padStart(4,'0')}`} editable={false} />
