...
export default function BanksScreen() {
  ...
  const handleSave = async () => {
    ...
    await getOrCreateParent('البنوك');
    const parent = accounts.find((a: any) => a.name === 'البنوك' && !a.parentId);
    if (parent) {
    ...
    Alert.alert('✅', 'تم إضافة البنك وإضافته للدليل تلقائياً');
  };
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '65%' }]}><View style={st.mh}><Text style={st.mt}>إضافة بنك</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
