...
export default function BanksScreen() {
  ...
  const handleSave = async () => {
    ...
    await addAccount({ name: formData.name, code, type: 'أصل', parentId: '112', balance: parseFloat(formData.balance) || 0, currency: formData.currency, isDebit: 1 });
    await loadAccounts();
    setShowModal(false); setFormData({ name: '', currency: 'YER', balance: '0' });
    Alert.alert('✅', `تم إضافة ${formData.name} تحت البنك في الدليل`);
  };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="البنوك" count={cashBoxes.length} onBack={() => router.back()} onAdd={() => { setFormData({ name: '', currency: 'YER', balance: '0' }); setShowModal(true); }} />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        ...
        <ScrollView style={st.mb}>
          ...
          <Text style={st.hint}>سيظهر تلقائياً في دليل الحسابات تحت "البنك"</Text>
