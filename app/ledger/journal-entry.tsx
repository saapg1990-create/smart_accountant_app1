...
export default function JournalEntryScreen() {
  ...
  const { data: entries, add } = useLocalTable('journalEntries');
  ...
  const handleSave = async () => {
    ...
    await add({ number: generateNumber(), date, description, totalDebit, totalCredit, lines: lines.filter(l => l.accountName) });
    setShowModal(false);
    Alert.alert('✅', 'تم حفظ القيد بنجاح');
  };
    ...
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      {filtered.length === 0 ? <View style={st.e}><Text style={st.ei}>📝</Text><Text style={st.et}>لا توجد قيود</Text></View> :
        <FlatList data={filtered} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.rc}>
            <View style={st.rh}><Text style={st.rn}>{item.number}</Text><Text style={st.rd}>{item.date}</Text></View>
            <Text style={st.rdesc}>{item.description}</Text><Text style={[st.rbal, { color: item.totalDebit === item.totalCredit ? '#10B981' : '#EF4444' }]}>مدين: {item.totalDebit?.toLocaleString()} | دائن: {item.totalCredit?.toLocaleString()}</Text>
          </TouchableOpacity>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        ...
        <ScrollView style={st.mb}>
          ...
          <Text style={st.st}>تفاصيل القيد (الحسابات الفرعية فقط)</Text>
          {lines.map((line, i) => (
          ...
          <View style={st.ss}><Text>مدين: {totalDebit.toLocaleString()} ﷼</Text><Text style={{ color: '#EF4444' }}>دائن: {totalCredit.toLocaleString()} ﷼</Text><Text style={{ color: isBalanced ? '#10B981' : '#EF4444' }}>{isBalanced ? '✅ متوازن' : '❌ غير متوازن'}</Text></View>
          <TouchableOpacity style={st.sb} onPress={handleSave}><Text style={st.sbt}>💾 حفظ القيد</Text></TouchableOpacity>
        </ScrollView></View></View>
      </Modal>
      <PickerModal visible={showAccountPicker} title="اختيار الحساب (الفرعية فقط)" data={leafAccounts} displayField="name" subField="code" onSelect={(i: any) => { updateLine(currentLineId, 'accountId', i.id); updateLine(currentLineId, 'accountName', i.name); }} onClose={() => setShowAccountPicker(false)} />
    </View>
  );
}
const st = StyleSheet.create({
  ...
  rc:{backgroundColor:'#16213E',borderRadius:14,padding:14,marginBottom:10,marginHorizontal:16,borderWidth:1,borderColor:'#2a3550'},rh:{flexDirection:'row',justifyContent:'space-between',marginBottom:6},rn:{color:'#D4AF37',fontSize:14,fontWeight:'bold'},rd:{color:'#94a3b8',fontSize:11},rdesc:{color:'#FFF',fontSize:13},rbal:{fontSize:12,fontWeight:'bold'},
  ...
  ss:{backgroundColor:'#0A1128',borderRadius:12,padding:14,marginTop:12},
