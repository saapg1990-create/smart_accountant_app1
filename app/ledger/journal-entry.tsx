...
export default function Screen() {
  ...
  const loadData = async () => { const res = await DataService.getJournalEntries(); setData(res); };

  const handleSave = async () => {
    if (!form.description.trim()) return Alert.alert('خطأ', 'أدخل البيان');
    await DataService.addJournalEntry({ id: 'journal-entry-' + Date.now(), ...form, number: 'journal-entry-' + Date.now().toString().slice(-6), amount: parseFloat(form.amount) || 0 });
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <View style={st.h}><TouchableOpacity onPress={() => router.back()}><Text style={st.bb}>←</Text></TouchableOpacity><Text style={st.t}>القيود</Text><TouchableOpacity onPress={() => setShowForm(true)}><Text style={st.add}>+</Text></TouchableOpacity></View>
      <TextInput style={st.si} placeholder="🔍 بحث..." placeholderTextColor="#666" value={searchQuery} onChangeText={setSearchQuery} />
      {showForm && (
        <Modal visible={showForm} animationType="slide" transparent>
          <View style={st.mo}><View style={st.mc}><View style={{flexDirection:'row',justifyContent:'space-between',padding:16}}><Text style={{color:'#D4AF37',fontSize:16,fontWeight:'bold'}}>القيود</Text><TouchableOpacity onPress={() => setShowForm(false)}><Text style={{color:'#EF4444',fontSize:22}}>✕</Text></TouchableOpacity></View>
      ...
      )} ListEmptyComponent={<Text style={st.et}>لا توجد القيود</Text>} />
    </View>
  );
}
