    ...
    if (balance > 0) {
      ...
      Alert.alert('تأكيد', `الرصيد: ${balance.toLocaleString()} ${formData.currency}\nالطبيعة: ${nature}`, [
        ...
        { text: 'حفظ', onPress: async () => {
          await addAccount({ name: formData.name, code, type: 'خصم', parentId: '211', balance: finalBalance, currency: formData.currency, isDebit: formData.isDebit ? 1 : 0, walletPhone: formData.phone, notes: formData.address });
          await loadAccounts(); resetForm(); setShowModal(false);
          Alert.alert('✅', `تم إضافة ${formData.name} تحت الموردين`);
        }}
      ]);
      return;
    }

    await addAccount({ name: formData.name, code, type: 'خصم', parentId: '211', balance: 0, currency: formData.currency, isDebit: 0, walletPhone: formData.phone, notes: formData.address });
    await loadAccounts(); resetForm(); setShowModal(false);
  };

  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="الموردين" count={customers.length} onBack={() => router.back()} onAdd={() => { resetForm(); setShowModal(true); }} />
