...
export default function UnitsScreen() {
  ...
  useFocusEffect(useCallback(() => { if(db) db.getAllAsync('SELECT * FROM units ORDER BY name').then(setData); }, [db]));

  const count = data.length + 1;
  const catNumber = `UN-${count.toString().padStart(4, '0')}`;

  const handleSave = async () => {
    if (!name.trim()) { Alert.alert('خطأ', 'أدخل اسم الوحدة'); return; }
    await db.runAsync('INSERT INTO units (id, name) VALUES (?,?)', ['cat-' + Date.now(), name]);
    db.getAllAsync('SELECT * FROM units ORDER BY name').then(setData);
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <ControlHeader title="الوحدات" count={data.length} onBack={() => router.back()} onAdd={() => { setName(''); setShowModal(true); }} />
      ...
      <FlatList data={data.filter((d: any) => d.name?.includes(searchQuery))} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
        <View style={st.rc}><Text style={st.rn}>📏 {item.name}</Text></View>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        ...
        <ScrollView style={st.mb}>
          ...
          <Text style={st.fl}>اسم الوحدة *</Text><TextInput style={st.fi} value={name} onChangeText={setName} />
