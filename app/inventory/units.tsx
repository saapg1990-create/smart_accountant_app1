...
export default function UnitsScreen() {
  ...
  const handleSave = async () => { if(!name.trim()){Alert.alert('خطأ','أدخل الاسم');return;} await db.runAsync('INSERT INTO units (id,name) VALUES (?,?)',['units-'+Date.now(),name]); db.getAllAsync('SELECT * FROM units ORDER BY name').then(setData); setName('');setShowModal(false); };
  return (<View style={[st.c,{paddingTop:insets.top}]}><ControlHeader title="الوحدات" count={data.length} onBack={()=>router.back()} onAdd={()=>setShowModal(true)} /><FlatList data={data} keyExtractor={i=>i.id} renderItem={({item})=>(<View style={st.rc}><Text style={st.rn}>📏 {item.name}</Text></View>)} ListEmptyComponent={<Text style={st.et}>لا توجد بيانات</Text>} contentContainerStyle={{padding:16}} />
