...
export default function PurchaseReturnScreen() {
  ...
  const { data: returns, add } = useLocalTable('purchaseReturns');
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', invoiceId: '', invoiceNumber: '', reason: '', total: '0' });
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="مردود المشتريات" count={returns.length} onBack={() => router.back()} onAdd={() => { setFormData({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', invoiceId: '', invoiceNumber: '', reason: '', total: '0' }); setLines([{ id: '1', itemId: '', itemName: '', qty: '0', price: '0', total: '0' }]); setShowModal(true); }} />
      ...
      <PickerModal visible={showItemPicker} title="اختيار الصنف" data={items || []} displayField="name" onSelect={(i: any) => { updateLine(currentLineId, 'itemId', i.id); updateLine(currentLineId, 'itemName', i.name); updateLine(currentLineId, 'price', i.cost?.toString() || '0'); }} onClose={() => setShowItemPicker(false)} />
    </View>
  );
}
