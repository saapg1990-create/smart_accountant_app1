...
export default function PurchaseReturnScreen() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', reason: '' });
  const [lines, setLines] = useState([{ id: '1', itemId: '', itemName: '', qty: '0', price: '0', total: '0' }]);

  useFocusEffect(useCallback(() => { loadAccounts(); }, []));

  const supplierAccounts = accounts.filter((a: any) => a.parentId === '211');
  ...
  const returnNumber = `PR-${count.toString().padStart(6, '0')}`;

  const handleSave = async () => {
    if (!formData.supplierName) { Alert.alert('خطأ', 'اختر المورد'); return; }
    
    const result = await unifiedPost('salesReturn', { ...formData, total, currency, exchangeRate, supplierId: formData.supplierId, supplierName: formData.supplierName });
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="مردود المشتريات" count={returns.length} onBack={() => router.back()} onAdd={() => { setFormData({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', reason: '' }); setLines([{ id: '1', itemId: '', itemName: '', qty: '0', price: '0', total: '0' }]); setShowModal(true); }} />
