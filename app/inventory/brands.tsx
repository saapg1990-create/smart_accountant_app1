...
export default function BrandsScreen() {
  ...
  useFocusEffect(useCallback(() => { if(db) db.getAllAsync('SELECT * FROM brands ORDER BY name').then(setData); }, [db]));

  const count = data.length + 1;
  const catNumber = `BR-${count.toString().padStart(4, '0')}`;

  const handleSave = async () => {
    if (!name.trim()) { Alert.alert('خطأ', 'أدخل اسم العلامة'); return; }
    await db.runAsync('INSERT INTO brands (id, name) VALUES (?,?)', ['cat-' + Date.now(), name]);
    db.getAllAsync('SELECT * FROM brands ORDER BY name').then(setData);
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <ControlHeader title="العلامات" count={data.length} onBack={() => router.back()} onAdd={() => { setName(''); setShowModal(true); }} />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={st.mc}><View style={st.mh}><Text style={st.mt}>إضافة علامة</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          <Text style={st.fl}>الرقم</Text><TextInput style={[st.fi,{color:'#D4AF37'}]} value={catNumber} editable={false} />
          <Text style={st.fl}>اسم العلامة *</Text><TextInput style={st.fi} value={name} onChangeText={setName} />
