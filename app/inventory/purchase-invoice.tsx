...
export default function PurchaseInvoiceScreen() {
  ...
  const { data: invoices, add } = useLocalTable('purchaseInvoices');
  ...
  const handleSave = async () => {
    ...
    await add({ number: generateNumber(), type: invoiceType, ...formData, subtotal, discount: discountAmount, total, paid, remaining, items: lines.filter(l => l.itemName) });
    setShowModal(false);
    Alert.alert('✅', 'تم حفظ فاتورة المشتريات بنجاح');
  };
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      {invoices.length === 0 ? <View style={st.e}><Text style={st.ei}>📄</Text><Text style={st.et}>لا توجد فواتير مشتريات</Text></View> :
        <FlatList data={invoices} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.rc}><Text style={st.rn}>{item.number}</Text><Text style={st.rd}>🏭 {item.supplierName}</Text><Text style={[st.rt, { color: item.type === 'cash' ? '#10B981' : '#F59E0B' }]}>{item.type === 'cash' ? '💰 نقدي' : '📋 آجل'} | {item.total?.toLocaleString()} ﷼</Text><Text style={{ color: '#EF4444', fontSize: 12 }}>المتبقي: {item.remaining?.toLocaleString() || 0} ﷼</Text></TouchableOpacity>
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        ...
        <ScrollView style={st.mb}>
          ...
          <Text style={st.fl}>رقم المرجع</Text><TextInput style={st.fi} value={formData.refNumber} onChangeText={v => setFormData({ ...formData, refNumber: v })} placeholder="رقم فاتورة المورد" placeholderTextColor="#666" />
          ...
          <TouchableOpacity style={st.sb} onPress={handleSave}><Text style={st.sbt}>💾 حفظ فاتورة المشتريات</Text></TouchableOpacity>
