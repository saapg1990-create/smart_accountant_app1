...
export default function PurchaseInvoiceScreen() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], supplierId: '', supplierName: '', cashId: '', cashName: '', warehouseId: '', warehouseName: '', paid: '0', discount: '0', description: '' });
  ...
  const handleSave = async () => {
    ...
    const result = await unifiedPost('purchaseInvoice', { ...formData, invoiceType, total, currency, exchangeRate, cashId: formData.cashId, cashName: formData.cashName, supplierId: formData.supplierId, supplierName: formData.supplierName });
