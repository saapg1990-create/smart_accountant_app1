...
export default function SalesInvoiceScreen() {
  ...
  const changeInvoiceType = (type: 'cash'|'credit') => {
    setInvoiceType(type);
    setFormData(prev => ({ ...prev, customerId: '', customerName: '', cashId: '', cashName: '' }));
  };

  const resetForm = () => {
    setFormData({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', cashId: '', cashName: '', warehouseId: '', warehouseName: '', paid: '0', discount: '0', description: '' });
