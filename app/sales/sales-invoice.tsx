...
export default function SalesInvoiceScreen() {
  ...
  const handleSave = async () => {
    ...
    await add({ number: generateNumber(), type: invoiceType, ...formData, subtotal, total, paid, remaining, items: lines.filter(l => l.itemName) });
    setShowModal(false);
    Alert.alert('✅', 'تم حفظ الفاتورة');
  };
    ...
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      {invoices.length === 0 ? <View style={st.e}><Text style={st.ei}>📄</Text><Text style={st.et}>لا توجد فواتير</Text></View> :
        <FlatList data={invoices} keyExtractor={(i: any) => i.id} renderItem={({ item }: any) => (
          <TouchableOpacity style={st.rc}><Text style={st.rn}>{item.number}</Text><Text style={st.rd}>👤 {item.customerName}</Text><Text style={[st.rt, { color: item.type === 'cash' ? '#10B981' : '#F59E0B' }]}>{item.type === 'cash' ? '💰 نقدي' : '📋 آجل'} | {item.total?.toLocaleString()} ﷼</Text></TouchableOpacity>
        )} contentContainerStyle={{ padding: 16 }} />}
      
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '95%' }]}><View style={st.mh}><Text style={st.mt}>فاتورة مبيعات</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
        <ScrollView style={st.mb}>
          ...
          {invoiceType === 'credit' && <><Text style={st.fl}>حساب العميل *</Text><TouchableOpacity style={st.pk} onPress={() => setShowAccountPicker(true)}><Text style={formData.accountName ? st.pkt : st.pkp}>{formData.accountName || 'اختيار حساب العميل'}</Text><Text style={st.pka}>▼</Text></TouchableOpacity></>}
          ...
          <View style={st.ss}><Text>الإجمالي: {subtotal.toLocaleString()} ﷼</Text><Text style={st.fl}>خصم</Text><TextInput style={st.fi} value={formData.discount} onChangeText={v => setFormData({ ...formData, discount: v })} keyboardType="numeric" /><Text style={st.fl}>المدفوع</Text><TextInput style={st.fi} value={formData.paid} onChangeText={v => setFormData({ ...formData, paid: v })} keyboardType="numeric" /><Text style={st.gt}>الصافي: {total.toLocaleString()} ﷼ | المتبقي: {remaining.toLocaleString()} ﷼</Text></View>
          <TouchableOpacity style={st.sb} onPress={handleSave}><Text style={st.sbt}>💾 حفظ</Text></TouchableOpacity>
        </ScrollView></View></View>
      </Modal>
      <PickerModal visible={showAccountPicker} title="اختيار حساب العميل" data={customerAccounts} displayField="name" subField="code" onSelect={(i: any) => setFormData({ ...formData, accountId: i.id, accountName: i.name })} onClose={() => setShowAccountPicker(false)} />
      ...
      <PickerModal visible={showItemPicker} title="اختيار الصنف" data={items || []} displayField="name" onSelect={(i: any) => { updateLine(currentLineId, 'itemId', i.id); updateLine(currentLineId, 'itemName', i.name); updateLine(currentLineId, 'price', i.salePrice?.toString() || '0'); }} onClose={() => setShowItemPicker(false)} />
...
const st = StyleSheet.create({
  ...
  al:{backgroundColor:'#D4AF37'+'20',borderRadius:10,padding:12,alignItems:'center',marginTop:8,borderWidth:1,borderColor:'#D4AF37'+'40'},ss:{backgroundColor:'#0A1128',borderRadius:12,padding:14,marginTop:12},gt:{color:'#F59E0B',fontSize:14,fontWeight:'bold',textAlign:'center',marginTop:8},
