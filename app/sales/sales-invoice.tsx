...
export default function SalesInvoiceScreen() {
  ...
  const [formData, setFormData] = useState({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', cashId: '', cashName: '', warehouseId: '', warehouseName: '', paid: '0', discount: '0', description: '' });
  const [lines, setLines] = useState([{ id: '1', itemId: '', itemName: '', qty: '0', price: '0', discount: '0', taxRate: 5, total: '0' }]);

  useFocusEffect(useCallback(() => { loadAccounts(); }, []));

  const addLine = () => setLines([...lines, { id: Date.now().toString(), itemId: '', itemName: '', qty: '0', price: '0', discount: '0', taxRate: 5, total: '0' }]);
  ...
  const taxAmount = lines.reduce((s, l) => s + ((parseFloat(l.qty)||0)*(parseFloat(l.price)||0)-(parseFloat(l.discount)||0))*((l.taxRate||5)/100), 0);
  const total = subtotal - totalDiscount + taxAmount - (parseFloat(formData.discount)||0);
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      <ControlHeader title="فواتير المبيعات" count={invoices.length} onBack={() => router.back()} onAdd={() => { setFormData({ date: new Date().toISOString().split('T')[0], customerId: '', customerName: '', cashId: '', cashName: '', warehouseId: '', warehouseName: '', paid: '0', discount: '0', description: '' }); setLines([{ id: '1', itemId: '', itemName: '', qty: '0', price: '0', discount: '0', taxRate: 5, total: '0' }]); setShowModal(true); }} />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        ...
        <ScrollView style={st.mb}>
          ...
          {lines.map((line, i) => (
            <View key={line.id}>
              <Selector label={i===0?"الصنف":""} tableName="items" displayField="name" selectedId={line.itemId} selectedName={line.itemName} onSelect={(item:any)=>{updateLine(line.id,'itemId',item.id);updateLine(line.id,'itemName',item.name);updateLine(line.id,'price',String(item.price||0))}} />
...
const st = StyleSheet.create({
  ...
  st:{fontSize:16,fontWeight:'bold',color:'#D4AF37',marginTop:16,marginBottom:10},
