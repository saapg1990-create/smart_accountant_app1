...
export default function CustomersScreen() {
  ...
  const cashBoxes = accounts.filter((a: any) => a.parentId === '114');
  ...
  const handleSave = async () => {
    ...
    await addAccount({ name: formData.name, code, type: 'أصل', parentId: '114', balance: parseFloat(formData.balance) || 0, currency: formData.currency, isDebit: 1 });
    await loadAccounts();
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}>
      <ControlHeader title="العملاء" count={cashBoxes.length} onBack={() => router.back()} onAdd={() => { setFormData({ name: '', currency: 'YER', balance: '0' }); setShowModal(true); }} />
