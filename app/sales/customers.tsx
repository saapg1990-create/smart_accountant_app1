...
export default function CustomersScreen() {
  ...
  const filtered = customers.filter((c: any) => (c.name || '').includes(searchQuery) || (c.phone || '').includes(searchQuery));
  ...
  const handleSave = async () => {
    ...
    await getOrCreateParent('العملاء');
    const parent = accounts.find((a: any) => a.name === 'العملاء' && !a.parentId);
    if (parent) {
    ...
    Alert.alert('✅', 'تم إضافة العميل وإضافته للدليل تلقائياً');
  };
  ...
  return (
    <View style={[st.c, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" />
      ...
      <Modal visible={showModal} animationType="slide" transparent>
        <View style={st.mo}><View style={[st.mc, { maxHeight: '75%' }]}><View style={st.mh}><Text style={st.mt}>إضافة عميل</Text><TouchableOpacity onPress={() => setShowModal(false)}><Text style={st.mx}>✕</Text></TouchableOpacity></View>
