...
export default function SalesReturnScreen() {
  ...
  const handleSave = async () => {
    ...
    await add({ number: 'SR-' + (returns.length + 1).toString().padStart(5, '0'), ...formData, total, items: lines.filter(l => l.itemName) });
    setShowModal(false); Alert.alert('✅', 'تم حفظ مردود المبيعات');
  };
