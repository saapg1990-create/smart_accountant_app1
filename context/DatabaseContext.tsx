  ...
  useEffect(() => {
    async function initDB() {
      try {
        ...
        await seed51Accounts(database);
        await seedCurrencies(database);
        setDb(database);
        setDatabase(database);
        console.log('✅ 51 حساب + أرصدة متعددة + 3 عملات');
...
export function useDatabase() {
  ...
  return ctx || { db: null, loading: true };
}
...
async function createAllTables(db: SQLite.SQLiteDatabase) {
  await db.execAsync(`CREATE TABLE IF NOT EXISTS accounts (
    ...
    parentId TEXT DEFAULT '', isDebit INTEGER DEFAULT 1,
    isActive INTEGER DEFAULT 1, bankAccount TEXT DEFAULT '',
    walletPhone TEXT DEFAULT '', notes TEXT DEFAULT ''
  )`);

  await db.execAsync(`CREATE TABLE IF NOT EXISTS account_balances (
    id TEXT PRIMARY KEY, account_id TEXT NOT NULL,
    currency TEXT DEFAULT 'YER', balance REAL DEFAULT 0,
    base_balance REAL DEFAULT 0, exchange_rate REAL DEFAULT 1,
  ...
  await db.execAsync(`CREATE TABLE IF NOT EXISTS vouchers (id TEXT PRIMARY KEY, number TEXT, type TEXT, voucherType TEXT, date TEXT, sourceName TEXT, accountName TEXT, amount REAL DEFAULT 0)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS salesInvoices (id TEXT PRIMARY KEY, number TEXT, date TEXT, customerId TEXT, type TEXT DEFAULT 'cash', total REAL DEFAULT 0)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS purchaseInvoices (id TEXT PRIMARY KEY, number TEXT, date TEXT, supplierId TEXT, type TEXT DEFAULT 'cash', total REAL DEFAULT 0)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS journal_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, number TEXT, date TEXT, description TEXT, type TEXT)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS journal_details (id INTEGER PRIMARY KEY AUTOINCREMENT, journal_id INTEGER, account_id TEXT, debit REAL DEFAULT 0, credit REAL DEFAULT 0)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS customers (id TEXT PRIMARY KEY, name TEXT, phone TEXT, address TEXT)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS suppliers (id TEXT PRIMARY KEY, name TEXT, phone TEXT, address TEXT)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS warehouses (id TEXT PRIMARY KEY, name TEXT, location TEXT DEFAULT '')`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS items (id TEXT PRIMARY KEY, name TEXT, code TEXT, unit TEXT DEFAULT 'حبة', cost REAL DEFAULT 0, price REAL DEFAULT 0, quantity REAL DEFAULT 0)`);
  ...
  await db.execAsync(`CREATE TABLE IF NOT EXISTS salesReps (id TEXT PRIMARY KEY, name TEXT, phone TEXT DEFAULT '', monthlyTarget REAL DEFAULT 0)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)`);
  await db.execAsync(`CREATE TABLE IF NOT EXISTS notifications (id TEXT PRIMARY KEY, type TEXT, title TEXT, message TEXT, read INTEGER DEFAULT 0)`);
}

async function seed51Accounts(db: SQLite.SQLiteDatabase) {
  const count = await db.getFirstAsync("SELECT COUNT(*) as c FROM accounts") as any;
  ...
  for (const acc of accounts) {
    await db.runAsync('INSERT OR IGNORE INTO accounts (id, code, name, type, parentId, isDebit, isActive) VALUES (?,?,?,?,?,?,1)', [acc.id, acc.code, acc.name, acc.type, acc.parentId, acc.isDebit]);
  }
  console.log('✅ 51 حساب');
}

async function seedCurrencies(db: SQLite.SQLiteDatabase) {
  const c = await db.getFirstAsync("SELECT COUNT(*) as c FROM currencies") as any;
  if (c?.c === 0) {
